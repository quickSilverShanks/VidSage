...
TZ_INFO = os.getenv("TIMEZONE", "Asia/Kolkata")
...
def get_db_connection():
    return psycopg2.connect(
        ...
        database=os.getenv("DB_NAME", "vidsage_tscripts"),
...
def save_conversation(conversation_id, video_id, question, response_data, timestamp=None):
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                ...
                (id, video_id, question, answer, model_used, response_time, relevance, 
...
def save_feedback(conversation_id, rating, feedback, timestamp=None):
    if timestamp is None:
        timestamp = datetime.now(tz)
    ...
    try:
        with conn.cursor() as cur:
            cur.execute(
                "INSERT INTO feedback (conversation_id, rating, feedback, timestamp) VALUES (%s, %s, %s, COALESCE(%s, CURRENT_TIMESTAMP))",
                (conversation_id, rating, feedback, timestamp),
            )
        conn.commit()
    finally:
        conn.close()
